#include "Game/SystemScheduler.h"
...
SystemScheduler::SystemScheduler(std::vector<ISystem*> systems, float maxSubStepDistance)
    : m_systems(std::move(systems))
    , m_maxSubStepDistance(maxSubStepDistance)
{
}
...
void SystemScheduler::Step(float deltaTime)
{
	...
	// Size the substep count so the fastest mover travels at most m_maxSubStepDistance per step.
	...
	int subSteps = 1;
	if (maxTravel > m_maxSubStepDistance)
		subSteps = static_cast<int>(std::ceil(maxTravel / m_maxSubStepDistance));
